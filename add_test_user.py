...
def add_test_data():
    ...
    if total_users > len(names):
        raise ValueError(
            "Not enough unique names available for the number of users")
    ...
    for tier in tiers:
        for i in range(5):
            ...
            name_index += 1
            test_users.append(
                {"user_id": user_id, "score": 100 * user_id, "tier": tier, "nickname": nickname})
            user_id += 1
        for i in range(5):
            nickname = names[name_index]
            name_index += 1
            test_users.append({"user_id": user_id, "score": 100 *
                              (user_id + 100), "tier": tier, "nickname": nickname})
            user_id += 1

    for user_data in test_users:
        existing_user = db.query(Ranking).filter(
            Ranking.user_id == user_data["user_id"]).first()
        if existing_user:
            existing_user.score += user_data["score"]
        else:
            new_user = Ranking(
                user_id=user_data["user_id"], score=user_data["score"], tier=user_data["tier"])
            db.add(new_user)

        # Create corresponding User entry if not exists
        existing_user_entry = db.query(User).filter(
            User.id == user_data["user_id"]).first()
