...
def read_users_by_tier(tier: Tier, db: Session = Depends(get_db)):
    ...
    if not users:
        raise HTTPException(
            status_code=404, detail="No users found in this tier")
    return users
...
def read_my_tier(request: Request, db: Session = Depends(get_db)):
    ...
    if not tier:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User tier not found")
...
def read_user_ranking(user_id: int, db: Session = Depends(get_db)):
    ...
    user_rank = next((i for i, user in enumerate(
        users, 1) if user.user_id == user_id), None)
...
def read_user_info(user_id: int, db: Session = Depends(get_db)):
    ...
    users_in_tier = db.query(Ranking).filter(
        Ranking.tier == user.tier).order_by(Ranking.score.desc()).all()
    # 특정 사용자의 티어 내 랭킹 찾기
    user_rank_in_tier = next((i for i, u in enumerate(
        users_in_tier, 1) if u.user_id == user_id), None)
